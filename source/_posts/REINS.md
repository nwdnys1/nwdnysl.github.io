...
- 进展
  ...
  - 1 先确定 nll 可以下降 并且下降后数据质量提升 2 再确定 mask 掩码是否起作用（W 是作用于掩码的 若掩码无效 W 就没法被学习）3 然后设计 W 的作用方式 看如何优化为无环图 比如离散采样 或是阈值剪枝 4 最后再对比实验 看看其它方案的效果 （是否需要自环？没有自环时 根节点的 attnoutput 是全 0 也就是说 输出向量和输入向量是一样的/如果有自回归 用 mask 还是用全 0 向量）
  - 若 nll 与数据质量并没有明显关联 有可能是 训练和采样的逻辑不符导致的 有几种修改的方式：1 训练改为串行 手动设置每一批中的非父节点向量是 mask 这样就和采样过程一致 2 用 mask 向量代表未知数据 那么采样时 根节点的 mha 输出的就是 mask 向量 然后输入给 ffn 训练时 如果没有替换 根节点的输出还是真实嵌入 不符 而如果刚好替换到根节点的嵌入 就和训练一致 我的理解是 当 mask 成功发生替换时 训练就可以让模型学会输入 mask 而输出正确的概率分布 如果替换概率够大 也许可以正确优化
  - 当 prob 调至 0.5 时 nll 和相关性距离有相关 接下来需要确定 mask 是否影响模型性能 使用 adult 数据集 发现 loss 差别不大 可能是简单的前馈神经网络就足以拟合数据 需要换更大的数据集（可以考虑专门针对特征比较多的大型数据集？） 并且需要考虑 transformer 在贝叶斯网络学习中的优势是什么 比如捕捉因果关系？
  - 那么这周可以专门针对 barley 和 statlog 两个大数据集 进行测试 包括测试 nll 和各个数据指标的关联 以及设计一个 W 的方案 看看是否能优化出无环图
  - barley 和 statlog 上 有无 mask 呈现出比较大的差异 接下来看看 nll 下降和指标的关系
  - 先用 64 8 2 0.5 参数 barley 在训练到 3000 代左右 nll 下降缓慢 最终收敛到 31.1 左右 JSD 和 nll 关联不大 相关性距离随 nll 下降 F1 分数上 前 1k 代明显较差 往后则差距不明显 DCR 同上 syn 收敛到-99 左右 WD 出现一开始比较好 后续反而上升 不过总计差距都不大 可以视为随机误差 相关性距离初期有明显下降趋势 后续则差别不大 R2 同理 DCR 同 barley 这样看起来像是采样的随机性比较大 导致有些时候比较符合训练 有些时候则不符合
  - 16 2 1 0.5 参数 barley 收敛到 37.1 左右 JSD/WD 相比参数一更小了 相关性距离则大许多 ML 指标也比参数一差许多 隐私则略好 syn 收敛到-54 左右 和 barley 差不多 说明模型复杂度有影响
  - 64 8 2 0.75 参数 调大 prob barley 收敛到 47.5 syn 收敛到-46 barley JSD WD 相关性距离竟然全都更小了 可能说明模型单纯的学到了输入 mask 后应该输出数据的概率分布？F1 和 R2 也显著提高 甚至超过 TRTR 与此同时 DCR 略微减小 这样看来 一种可能是 prob 提高后 训练和采样的逻辑更加匹配 所以质量提高 另一种可能则是 嵌入层的效果很差 因此甚至不如我们直接输入 mask 向量给模型
  - 64 8 2 1 参数 我们干脆试试全是 mask 向量的情况 发现 JSD WD 都缩小 相关性距离变大 至于 R2 和 F1 则惨不忍睹
  - 64 8 2 0.9 参数 barley 收敛到 67.8 syn 收敛到 7 左右 和 1 的情况差不多 说明 prob 并不是越大越好 那么就不是嵌入层的问题
  - 64 8 2 0.5 0.1 0.1 采用 GS 采样 图是有效的 barley 收敛到 32 左右 syn 收敛到-95 左右 结果与非 GS 相比有好有坏 差不多 有可能是训练未收敛 也有可能是 prob 不够高
  - 64 8 2 0.75 0.1 0.1 GS barley 收敛到 60.5 syn 收敛到-3.7 看起来没有收敛 并且也没有接近无环图 指标并没有更好 说明是训练不够久
  - 接下来试试 64 8 2 0.5 0.1 0.1 GS 训练 1000000 代 看看能否收敛 可以跑出无环的图 barley 训练到 15w 代时 JSD 与相关性距离都与非 GS 时差不多 F1 则好的多 甚至超过 SMOTE 达到最优 DCR 与非 GS 差不多 syn 训练到 7w 代 WD 与非 GS 差不多 相关性距离和 R2 略差于非 GS DCR 差不多 可能是未收敛
  - 先看看怎么加快训练 现在收敛的太慢了
  - 采样时应该用一个确定的 W 需要改一下
  - 训练速度是否更快
  - 边保留了多少
...
- 
- 64 8 2 0.75 参数下：barley 的 JSD 和相关性距离都是最优 F1 应该仅次于 SMOTE DCR 应该位于中游 syn 的 WD 相关性距离 R2 大概位于第二或第三 DCR 同样位于中游 在没有使用 GS 采样的情况下 这个 prob 是比较优的选择 此时的模型没有因果顺序 矩阵 W 完全是一个全连接图

  - 使用 Gumbel-Softmax 采样 矩阵 W 可以向无环图方向优化 在 64 8 2 0.5 0.1 0.1 参数下 barley 的指标接近于非 GS 采样时 syn 则略逊于非 GS 采样 比较奇怪的是 W 优化为无环图并不能带来更优的相关性距离 可能是因为前者的 W 就是一个全连接图 保留了所有信息？

  - 推测如果采用合适的 prob 可以做到指标最优 且矩阵 W 能优化为无环图 接下来的工作是尝试解决训练较慢的问题 然后寻找合适的超参数
